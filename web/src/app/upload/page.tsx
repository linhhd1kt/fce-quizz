...
export default function UploadPage() {
  ...
  async function handleFile(file: File) {
    ...
    setStatus('loading'); setMessage(''); setQuiz(null); setSaved(false);
    ...
    try {
      ...
      setStatus('success');
      setMessage(i(m.foundMsg, { count: data.totalQuestions, title: data.title }));
    } catch { setStatus('error'); setMessage('Network error. Please try again.'); }
  }

  function handleSave() { if (!quiz) return; saveImportedQuizSet(quiz); setSaved(true); }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-3">
        <p className="text-xs font-semibold uppercase tracking-widest text-slate-500">{m.optionsLabel}</p>
        <label className="flex items-center gap-3 cursor-pointer">
          ...
          <span className="text-sm text-slate-300">
            {m.autoDetect}<span className="text-slate-500 ml-1.5">{m.autoDetectHint}</span>
          </span>
        </label>
        <div className="flex items-center gap-3">
          <span className="text-sm text-slate-300 shrink-0">{m.pageRangeLabel}</span>
          <input type="text" placeholder={m.pageRangePlaceholder} value={pageRange}
        ...
        <p className="text-xs text-slate-600">{m.pageRangeHint}</p>
      </div>
      ...
      <div onDrop={handleDrop} onDragOver={(e) => e.preventDefault()}
        ...
        {isLoading ? (
          <><div className="text-4xl mb-3 animate-pulse">⚙️</div>
            <p className="text-blue-400 text-sm font-medium">{m.extracting}</p>
            <p className="text-slate-500 text-xs mt-1">{m.extractingHint}</p></>
        ) : (
          <><p className="text-4xl mb-3">📄</p>
            <p className="text-slate-400 text-sm">{m.dropZone}</p>
            <p className="text-slate-600 text-xs mt-1">{m.maxSize}</p></>
        )}
      ...
      {status === 'error' && (
        <div className="bg-red-950 border border-red-800 rounded-xl p-4">
          <p className="text-red-400 text-sm">{m.errorPrefix}{message}</p>
        </div>
      )}
      ...
      {status === 'success' && quiz && (
        <div className="space-y-4">
          <div className="bg-emerald-950 border border-emerald-800 rounded-xl p-4 flex items-center justify-between gap-4">
            <p className="text-emerald-400 font-semibold text-sm">{m.successPrefix}{message}</p>
            <div className="flex gap-2 shrink-0">
              {!saved ? (
                <button onClick={handleSave} className="px-4 py-1.5 bg-emerald-700 hover:bg-emerald-600 text-white text-sm font-semibold rounded-lg transition-colors">{m.saveToLibrary}</button>
              ) : (
                <Link href={`/quiz/${quiz.id}`} className="px-4 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold rounded-lg transition-colors">{m.startQuiz}</Link>
              )}
            </div>
          </div>
          <div className="space-y-2">
            <p className="text-xs font-semibold uppercase tracking-widest text-slate-500">{i(m.previewLabel, { count: quiz.questions.length })}</p>
            {quiz.questions.slice(0, 5).map((q, idx) => (
              <div key={q.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-2">
                ...
                {q.explanation && <p className="text-xs text-slate-500 italic border-t border-slate-800 pt-2">{q.explanation}</p>}
              </div>
            ))}
            {quiz.questions.length > 5 && <p className="text-center text-slate-600 text-xs">{i(m.moreQuestions, { count: quiz.questions.length - 5 })}</p>}
          </div>
        </div>
      )}

      <div className="flex gap-4 text-sm text-slate-500">
        <Link href="/" className="hover:text-slate-300">{m.backHome}</Link>
        <Link href="/import" className="hover:text-slate-300">{m.importJson}</Link>
      </div>
    </div>
  );
}
