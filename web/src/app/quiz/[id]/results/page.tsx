...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    const found = attemptId
      ? all.find((a) => a.id === attemptId)
      : all.find((a) => a.quizId === id);
    ...
    <div className="max-w-lg mx-auto space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center space-y-3">
        ...
        <div className="flex justify-center gap-6 text-sm text-slate-500 pt-2">
          <span>{correct} / {attempt.totalQuestions} correct</span>
          <span>{formatTime(attempt.totalTimeSpent)} total</span>
        </div>
      </div>

      {/* Per-question review */}
      <section className="space-y-2">
        <h2 className="text-xs font-semibold uppercase tracking-widest text-slate-500">Review</h2>
        {attempt.answers.map((a, i) => (
          ...
          >
            ...
            <span className="flex-1">{a.correct ? '✓ Correct' : `✗ Wrong — answered: ${a.selected || '(no answer)'}`}</span>
      ...
      <div className="flex gap-3">
        <Link
          href={`/quiz/${id}`}
          className="flex-1 text-center px-4 py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-xl text-sm transition-colors"
        >
          Try Again
        </Link>
        <Link
          href="/"
          className="flex-1 text-center px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-white font-semibold rounded-xl text-sm transition-colors"
        >
          ← Home
        </Link>
      </div>
